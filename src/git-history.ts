...
export const gitHistory = async (gitFolder: string, tmpFile: string): Promise<boolean> => {
    /*
    ...
    await git.addConfig("user.name", "Is-Academia-Bot").addConfig("user.email", "noreply@github.com");

    /*
     * Change or add remote if not exist
     */
    const remoteURL = transformGitUrl(env.GIT_USERNAME, env.GIT_PASSWORD, env.GIT_REMOTE);
     ...
     */
    await git.fetch();
    const branchExists = await git.branchLocal().then((result) => result.all.includes(env.GIT_BRANCH));
    if (!branchExists) {
     ...
     */
    const response = await git.commit(`Update ${format(new Date(), "yyyy-MM-dd-HH-mm")}`);
    await git.push("origin", env.GIT_BRANCH);
    logger.info("Changes have been committed and pushed to the remote repository.");
