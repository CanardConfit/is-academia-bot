 ...
 */
export const telegramSendUpdates = (token: string, chatId: string, differences: Difference[]) => {
    ...
    differences.forEach((difference, index) => {
        ...
        bot.telegram
            ...
            .then(() => {
                logger.info(`Telegram service : successfully sent update ${index + 1} of ${differences.length}.`);
            })
            .catch((error) => {
                logger.error(`Telegram service : failed to send update ${index + 1} of ${differences.length}. Error: ${error}`);
            });
    });
};
